...
Create a dashboard JSON file and then import it:

```bash
PROJECT_ID=$(gcloud config get-value project)
ZONE="us-central1-a"

cat > /tmp/lab10-dashboard.json << 'EOF'
{
  "displayName": "Lab 10 — GCE Operations Dashboard",
  "gridLayout": {
    "widgets": [
      {
        "title": "CPU Utilization — lab10-vm",
        "xyChart": {
          "dataSets": [
            {
              "timeSeriesQuery": {
                "timeSeriesFilter": {
                  "filter": "metric.type=\"compute.googleapis.com/instance/cpu/utilization\" resource.type=\"gce_instance\" metadata.user_labels.\"name\"=\"lab10-vm\"",
                  "aggregation": {
                    "alignmentPeriod": "60s",
      ...
      {
        "title": "Network Bytes In/Out — lab10-vm",
        "xyChart": {
          "dataSets": [
            {
              "timeSeriesQuery": {
      ...
      {
        "title": "Disk Read/Write Operations — lab10-vm",
        "xyChart": {
          "dataSets": [
            {
              "timeSeriesQuery": {
...
gcloud monitoring dashboards create \
  --config-from-file=/tmp/lab10-dashboard.json \
  --project="${PROJECT_ID}"
```
...
cat > /tmp/lab10-cpu-alert.json << 'EOF'
{
  ...
  "enabled": true,
  "documentation": {
    "content": "CPU utilization on lab10-vm has exceeded 80% for more than 5 minutes. Investigate running processes with: gcloud compute ssh lab10-vm -- top -bn1 | head -20",
...
gcloud alpha monitoring policies create \
  --policy-from-file=/tmp/lab10-cpu-alert.json \
  --project="${PROJECT_ID}"
```
