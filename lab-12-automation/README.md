...
steps:
  ...
  # Step 2: Run a smoke test — start the container and curl /health
  # This step intentionally uses a network to allow the curl step to reach the app
  - name: 'gcr.io/cloud-builders/docker'
    id: 'start-for-test'
    args:
      - 'run'
  ...
  - name: 'curlimages/curl:8.5.0'
    ...
    args:
      ...
      - |
        sleep 2
        curl -sf http://lab12-test:8080/health && echo "Smoke test passed"
    waitFor: ['start-for-test']

  # Step 3: Stop and remove the test container
  - name: 'gcr.io/cloud-builders/docker'
    id: 'stop-test-container'
    args: ['rm', '-f', 'lab12-test']
    waitFor: ['smoke-test']
...
Step #0 - "build": Successfully built abc123def456
Step #0 - "build": Successfully tagged us-central1-docker.pkg.dev/.../lab12-app:abcdef1
Finished Step #0 - "build"
Starting Step #1 - "start-for-test"
...
Starting Step #2 - "smoke-test"
Step #2 - "smoke-test": ok
Step #2 - "smoke-test": Smoke test passed
Finished Step #2 - "smoke-test"
...
PUSH
Finished Step #4 - "push-sha"
...
DONE
```
